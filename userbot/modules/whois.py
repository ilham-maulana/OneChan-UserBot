...
async def who(event):
    ...
    await event.edit(
        "Mencari Informasi Yang Lengkap Dan Akurat\n**Creator:** [NFS乛Rizky戈手]")
...
async def fetch_info(replied_user, event):
    ...
    first_name = first_name.replace(
        "\u2060", "") if first_name else ("Tidak Ada Nama Pertama")
    last_name = last_name.replace(
        "\u2060", "") if last_name else ("Tidak Ada Nama Akhir")
    username = "@{}".format(username) if username else (
        "Tidak Mempunyai Username")
    user_bio = "This User has no About" if not user_bio else user_bio

    caption = "<b>INFO SI BANGSAT:</b>\n\n"
    caption += f"Nama Pertama: {first_name}\n"
    caption += f"Nama Akhir: {last_name}\n"
    caption += f"Username: {username}\n"
    caption += f"Data ID: {dc_id}\n"
    caption += f"Jumlah Foto Profil: {replied_user_profile_photos_count}\n"
    caption += f"Apakah Bot: {is_bot}\n"
    caption += f"Apakah Restricted: {restricted}\n"
    caption += f"Terferifikasi oleh Telegram: {verified}\n"
    caption += f"ID: {user_id}\n\n"
    caption += f"Bio: \n{user_bio}\n\n"
    caption += f"Jumlah Grup Yang Sama: {common_chat}\n"
    caption += f"Link Permanen Profile: "
